...
1. `Load_QC_input_files` This path defines where to find your scRNA file(s). Options can be a single file or a directory which contains several files. For each pair of CellRanger file (barcodes.tsv.gz, features.tsv.gz, matrix.mtx.gz), they should be placed in distinct directories under Load_QC_input_files. For other format (.h5 and .txt), they can be placed under Load_QC_input_files without creating any directory.
