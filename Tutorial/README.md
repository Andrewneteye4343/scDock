...
● Data preparation:  
Please manually download those data to your computer. Please rename the files as illustrated in the example below for 10x Cell Ranger data. Each sample directory should contain the following files:  
`barcodes.tsv.gz`  
`features.tsv.gz`  
`matrix.mtx.gz`  
...
3. `Load_QC_max_mito` This argument defines the percent of mitochondrial counts cut off in subset(). The valid range locates from 0 to 1. Default is 0.1. Lower cut off will remove more cells in the further analysis.
4. `Load_QC_metadata_file` This path defines where to find metadata for grouping your samples.  For example: /home/andrew/scDock/metadata.txt (Optional)  
5. `Run_Integration_run_integration` This argument defines whether to perform data integration. Options are true and false.  
6. `Run_Integration_method` This argument defines the method in Seurat::IntegrateLayers(). The valid values are "cca", "harmony" and "rpca". Default is "cca". Should be modified only if you intend to run integration (Optional).  
7. `Run_Integration_dims` This argument defines how many PCs will be used in the dimensional reduction analysis. Default is "auto". The default setting will use a proper PC number based on geometric Elbow method. Should be modified only if you intend to run integration (Optional).  
8. `Run_Integration_resolution` This argument defines the resoltion in Seurat::FindClusters(). Default is 0.5. Higher resolution might bring more clusters in results. Should be modified only if you intend to run integration (Optional).  
9. `Markers_Annotation_tissue_type` This argument defines the tissue type of your scRNA/snRNA data. Please remember to set this correctly.
10. `Run_CellChat_group_by` This argument defines the group.by in CellChat::createCellChat(). Default is "Celltype". If you are interested in groups from metadata.txt, you can set this argument with "sample_group".  
11. `Run_CellChat_ntop_signaling` Choose how many top signalings (detailed L-R pairs in each pathway) you want to explore. More signalings will increase the time cost for further molecular docking. Defualt is 5.
12. `Vina_Docking_ligand_ref_file` and `Vina_Docking_receptor_ref_file` These arguments defines where to find the ligand_reference.csv and receptor_reference.csv that we provided in [functions](https://github.com/Andrewneteye4343/scDock/tree/main/Essential_files/functions).
13. `Vina_Docking_cas_txt_file` This argument specifies the location of cas.txt. You may provide a text file containing the CAS numbers of the compounds you are interested in, with one number per line. If this file is provided, the program will ignore both use_fda and fda_txt_path.  
14. `Vina_Docking_vina_exhaustiveness` This argument defines the exhaustiveness in AudoDock Vina. The higher value will increase the exhaustiveness of the global search, but also increase the time cost. Default is 8.
15. `Vina_Docking_vina_cpu` This argument defines the how many cpu(s) will be used in AutoDock Vina. Default is 1.  

● Start tutorial:  
Time cost: approximately 50 minutes. This tutorial processes ~27,000 cells and 20,695 genes after QC and perfomrs Harmony integration. During the docking step, two proteins and 43 compounds were docked with argument `Vina_Docking_vina_exhaustiveness` set to eight, using a single CPU.  
CPU and Memory Usage: The CPU usage reached 274%, and the maximum resident set size was approximately 14.59 GB.  
1. If you haven’t installed scDock, run `bash install.sh`. The [installation](https://github.com/Andrewneteye4343/scDock/tree/main/Installation) should complete within a few minutes.  
...
7. After marker identification and cell annotation are completed, the corresponding results will be saved. If the data originates from mouse samples, gene names will be converted to their human homologs before cell annotation.<img width="959" height="188" alt="Figure 7" src="https://github.com/user-attachments/assets/796e371c-81b0-40e1-832c-2ef1cd0050c1" />  
...
Bubble_plot_maxGroup_Control.pdf, Bubble_plot_maxGroup_DN.pdf, Incoming_signaling.pdf, Outgoing_signaling.pdf, and multiGroup_significant_LR.csv.  <img width="1891" height="144" alt="Figure 8" src="https://github.com/user-attachments/assets/583ca4a4-40fb-4239-a6f2-fa7aa6008985" />  <img width="1896" height="142" alt="Figure 9" src="https://github.com/user-attachments/assets/2d1230d4-9607-48ef-a2bc-85b3d648e6e3" />  <img width="1182" height="340" alt="Figure 10" src="https://github.com/user-attachments/assets/bb39374c-ca1f-407f-b5d7-b91374ac6dda" />  
9. Before molecular docking, scDock retrieves the PDB IDs of ligands and receptors involved in the top cellular signaling interactions between groups. It then downloads the corresponding protein structures from the Protein Data Bank (PDB) and performs preprocessing for docking preparation. Afterward, scDock downloads and preprocesses the compounds specified by the user. The final prepared structures are saved as .pdbqt files.  <img width="1083" height="517" alt="Tutorial_figure_GitHub" src="https://github.com/user-attachments/assets/fb88d31c-ca4b-47b6-ba9b-573fc5eefa90" />
...
<img width="392" height="691" alt="Tutorial_figure_GitHub_3" src="https://github.com/user-attachments/assets/8733a0d6-31ce-4e76-9915-85c7caffb37b" />
...
7. Bubble_plot_maxGroup_Control.pdf and Bubble_plot_maxGroup_DN.pdf – Visualize signaling pathways that are more active in a specific group (e.g., DN) compared to others. [Bubble_plot_maxGroup_DN.pdf](https://github.com/user-attachments/files/23565989/Bubble_plot_maxGroup_DN.pdf)
...
10. Output_drug_information.csv – Record the drug annotation of compounds used in molecular docking. In default, it contains CAS number and PubChem CID. If user set Drug_Information parameter to Curated_drug_information_BindingDB.tsv, this .csv file contains CAS number, PubChem CID, drug name in BindingDB, known targets, target organism, experimentally measured affinity values, and direct links to the corresponding ligand, target, and ligand–target interaction records in BindingDB.  
11. CellChat_ligand/ – Contains the protein structures (ReceptorName_prepared.pdbqt) and detailed docking results (LigandName_result_structure.pdbqt & AutoDockVina_score.csv) of ligand proteins listed in ligands_with_PDB.csv. In AutoDockVina_score.csv, scDock ranks the compounds by their binding affinity from lowest to highest.
12. CellChat_receptor/ – Contains the protein structures (ReceptorName_prepared.pdbqt) and detailed docking results (LigandName_result_structure.pdbqt & AutoDockVina_score.csv) of receptor proteins listed in receptors_with_PDB.csv.
13. ligand_structures_from_CAStxt_for_AutoDockVina/ – Stores the prepared compound structures for molecular docking provided in cas.txt. 
14. seurat_output/ – Records the suerat object files (.rds) from the intermediate steps in scRNA analysis.
