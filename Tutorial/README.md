...
2. `Run_Integration_method` This optional argument defines the method in Seurat::IntegrateLayers(). The valid value are "cca", "harmony" and "rpca". Default is "cca".  
3. `Run_Integration_dims` This optional argument defines how many PCs will be used in the dimensional reduction analysis. Default is "auto". The default setting will use a proper PC number based on geometric Elbow method. Should be modified only if you intend to run integration.  
4. `Run_Integration_resolution` This optional argument defines the resoltion in Seurat::FindClusters(). Default is 0.5. Higher resolution might bring more clusters in results. This argument is optional. Should be modified only if you intend to run integration.  
