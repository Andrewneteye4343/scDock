...
11. `Vina_Docking_ligand_ref_file` and `Vina_Docking_receptor_ref_file` These arguments defines where to find the ligand_reference.csv and receptor_reference.csv that we provided in [functions](https://github.com/Andrewneteye4343/scDock/tree/main/Essential%20files/functions).
...
● Start tutorial:  
##### Time cost: approximately 50 minutes. This tutorial processes ~27,000 cells and 20,695 genes after QC. During the docking step, two proteins and 43 compounds were docked with vina_exhaustiveness set to eight, using a single CPU. The CPU usage reached 274%, and the maximum resident set size was approximately 14.59 GB.  
